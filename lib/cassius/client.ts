import type { IntelligenceAnswer, IntegrationResult } from '../collective.ts';

export async function askCassius(question: string, fetcher: typeof fetch = fetch): Promise<IntegrationResult<IntelligenceAnswer>> {
  try {
    const response = await fetcher('/api/cassius', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ question }), signal: AbortSignal.timeout(35000),
    });
    ...
    return { state: 'ready', data: { text: data.text, citations: data.citations }, source: data.mode === 'openai' ? 'OpenAI / GGC knowledge' : 'GGC evidence boundary', updatedAt: new Date().toISOString() };
  } catch {
