...
const company = /\b(ggc|gc|groomed gent|collective|ambassador|commission|payout|referral|our (?:new )?(?:brand|products?|polic\w*|company)|the brand|internal polic\w*|hydros|santal noir)\b/i;
const research = /\b(grooming|beard|shaving|skincare|skin care|hair care|ingredients?|supplements?|scientific|clinical|efficacy|curriculum)\b/i;
const broad = /\b(travel|trip|itinerary|business|brainstorm|write|draft|plan|food|recipe|fitness|technology|explain|weekend|workout)\b/i;

/** Routing optimizes retrieval, never grants permission to invent company facts. */
export function conversationContext(question: string, history: ConversationTurn[] = []) {
  // Follow-ups may depend on a product named earlier; an explicit new topic stands alone.
  const followup = /\b(it|its|that|those|them|these|this product|same|also|what about|how much|tell me more)\b/i.test(question);
  const previous = history.filter(t => t.role === 'user').slice(-3).map(t => t.content).join('\n');
  ...
  const mode: ConversationMode = proprietary ? (broad.test(question) ? 'mixed' : 'authoritative') : 'general';
  ...
  return { mode, question, knowledge: {
    version: context.version, reviewedAt: context.reviewedAt, state: context.state, healthCurriculumVersion: 'healthCurriculumVersion' in context ? context.healthCurriculumVersion : undefined,
    passages: context.passages,
    limitation: 'reason' in context ? context.reason : context.nonEvidenceResponse,
  } };
}

export const conversationInstructions = `You are Cassius, an elite AI concierge and advisor for Groomed Gent Collective ambassadors: a gentleman AI in your pocket. Speak with quiet confidence, warmth, refinement and practical good sense. Be natural, useful and conversational, without pompous formality. Follow the user's requested style and level of detail.
