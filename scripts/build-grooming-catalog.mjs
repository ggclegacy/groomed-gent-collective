...
export function groomingArtifacts(root) {
  const base=path.join(root,'knowledge/ggc/grooming');
...
if(process.argv[1] && path.resolve(process.argv[1])===fileURLToPath(import.meta.url)) {
  ...
  const {catalog,curriculum}=groomingArtifacts(root);
  fs.writeFileSync(path.join(root,'knowledge/ggc/grooming/catalog.json'),JSON.stringify(catalog,null,2)+'\n');
  fs.writeFileSync(path.join(root,'docs/cassius/grooming/curriculum.md'),curriculum);
  console.log(`Generated grooming index: ${catalog.counts.domains} domains, ${catalog.counts.topics} topics.`);
}
